...
if [ $ramdsize -gt $count ]; then
	echo "##### Your initramfs needs to be compressed!! ###"
	cp out/new_ramdisk.cpio out/ramdisk.backup
	echo "##### Compressing your initramfs with lzma ###"
	cat out/ramdisk.backup | lzma -f -9 > stage1/compressed-ramdisk.cpio.lzma
	pushd stage1
	find ./ | cpio -o -H newc > $OUT/new_ramdisk.cpio
	popd
	ramdsize=`ls -l out/new_ramdisk.cpio | awk '{print $5}'`
	echo "##### 06. The size of the new compressed ramdisk is = $ramdsize"
else
